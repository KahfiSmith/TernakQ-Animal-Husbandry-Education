        ...
        <form method="POST" :action="`/populasi/${editData.id}`">
            ...
            <div class="space-y-4 mb-8">
                ...
                <div>
                    ...
                    <select id="kandang_id" name="kandang_id" class="ring-2 ring-gray-700 shadow-[4px_4px_0px_2px_#374151] focus:shadow-[2px_2px_0px_2px_#374151] focus:translate-y-0.5 focus:translate-x-0.5 rounded-md focus:outline-none focus:border-none focus:ring-2 focus:ring-gray-700 text-gray-700 leading-5 transition duration-150 ease-in-out block mt-1 w-full py-2.5" required x-model="editData.kandang_id">
                        <option value="" disabled selected>Pilih Kandang</option>
                        @foreach ($kandang as $item)
                            <option value="{{ $item->id }}">{{ $item->nama_kandang }} ({{ $item->kapasitas }} Kapasitas)</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <div class="flex space-x-4">
                        <x-text-input id="batchPrefix" name="batchPrefix" type="text" class="w-1/4" value="BATCH-" readonly />
                        <x-text-input id="batchCodeSuffix" name="batchCodeSuffix" type="text" class="w-full" required
                ...
                <!-- Input Nama Batch -->
                <div>
                    <x-input-label for="nama_batch" :value="__('Nama Batch')" />
                    <x-text-input id="nama_batch" name="nama_batch" type="text" class="block mt-1 w-full py-2.5"
                                  required x-model="editData.nama_batch"/>
                </div>
                ...
                <div>
                    ...
                    <x-text-input id="tanggal_doc" name="tanggal_doc" type="date" class="block mt-1 w-full py-2.5"
                                  required x-model="editData.tanggal_doc" />
                </div>

                <!-- Input Jumlah Ayam Masuk -->
                <div>
                    <x-input-label for="jumlah_ayam_masuk" :value="__('Jumlah Ayam Masuk')" />
                    <x-text-input id="jumlah_ayam_masuk" name="jumlah_ayam_masuk" type="number"
                                  class="block mt-1 w-full py-2.5" required x-model="editData.jumlah_ayam_masuk" />
                </div>
            ...
            <div class="flex items-center justify-end">
                <x-primary-button class="ring-2 ring-gray-700 shadow-[4px_4px_0px_2px_#374151] text-white hover:shadow-[2px_2px_0px_2px_#374151] hover:translate-y-0.5 hover:translate-x-0.5 w-1/2 text-center bg-orangeCrayola py-2.5">
                    {{ __('Perbarui Data') }}
                </x-primary-button>
            </div>
        </form>
